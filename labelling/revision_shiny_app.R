...
")),
  ...
  tabsetPanel( # to create tabs
    ...
    tabPanel("Start",
             ...
             h2("Welcome to the Chondrogenesis Revision Labeling App", 
       ...
       and those that do not."),
             p("Specifically, this app is designed to achieve a consensus between already labelled
               sentences with disagreements or unclear labelling"),
             hr(style = "border: 1px solid #bdc3c7; margin: 20px 0;"),  # Grey horizontal line
             
             h3("What is your role in the app?"),
             p("The group of curator will assemble and chose which label corresponds better to the gene"),
             hr(style = "border: 1px solid #bdc3c7; margin: 20px 0;"),  # Grey horizontal line
             ...
             h3("How do you chose which file to star labelling?"),
             p("Go to the ",
               strong("Upload File"),
               " window to chose which file to read. Be careful when choosing, the file of labelling needs to be in the 
               same folder as this app script to save the results — this step is very important!"),
             hr(style = "border: 1px solid #bdc3c7; margin: 20px 0;"),  # Grey horizontal line
    ...
    # User Window ####
    tabPanel("Upload File", id = "user-tab", # We add an ID so the tags are working only in this tab
             div(
               style = "max-width: 900px; margin: 0 auto; padding: 20px 0; font-size: 1.1em;",
               h2("Selection File to Label", style = "color:#2196F3"),
               HTML("Make sure that the file that you are going to handle is in the same folder as this app script"),
               # Create a container for centering the selectInput
               br(),
               br(),
               div(style = "margin-bottom: 5px;",
                   tags$label("Choose CSV Revision file", 
                              `for` = "revisionFile", 
                              style = "font-weight: bold; display: block; font-size:30"),
                   fileInput("revisionFile",
                             label = NULL,
                             accept = ".csv",
                             placeholder = "Choose revision file",
                             width = "100%"),
                             buttonLabel = tags$strong("Browse")),

               HTML("If you want to be add as a curator send a message to <i>a.valdes@liverpool.ac.uk</i>")
               )
            )
             ,
    # Label Window ####
    tabPanel("Label", id = "labelling-tab", #name of the tab
             ...
             fluidRow(
                 ...
                 # This is going to be the already labelled results from other curators
                  column(3,
                         class="side-panel",
                         h3("Curators Choices"),
                         htmlOutput("previousChoices"))
              ),
    ...
    tabPanel("Instructions",
             ...
             bsCollapse(id = "collapsePanels", open = NULL,
                          ...
                          bsCollapsePanel("Curators Section", 
                                          h3("Overview", style = "font-size: 23px;"),
                                          p("This section contains the previous choices of all the curators that are featured in the uploaded file regarding the options of the highlighted gene.", style = "font-size: 16px;"),
                                          img(src = "curators_choices.png", style = "max-width: 100%; border-radius: 8px; box-shadow: 2px 2px 10px #ccc;text-align:cente;")
                          )
               ),
               ...
               div(style = "background-color: #ffffff; padding: 15px; border-radius: 10px; 
                 ...
                 p("Select the file that is formated for the revision app and make sure that this file is in the same folder as the app script or the results will not be saved correctly. If there are no sentences left to label in that file, a warning will appear in the bottom right corner of the window."),
               ),
               br(),
               div(style = "background-color: #ffffff; padding: 15px; border-radius: 10px; 
                 box-shadow: 4px 4px 10px rgba(0, 0, 0, 0.1);
    ...
    tabPanel("Rules", #name of the tab
             ...
             div(
               ...
               div(
                 ...
                 box-shadow: 4px 4px 10px rgba(0, 0, 0, 0.1); width: 300px; text-align: center; 
                 border: 2px solid #bdc3c7; color: #2c3e50;",
                 h4("Exclusion of Cell Proliferation and Repair", style = "color: #2c3e50; font-size: 20px; font-weight: bold;"),
                 p("Genes related to only cell proliferation or cartilage repair should not be labeled as related to chondrogenesis."),
                 h4("Example of Rule", style = "color: #5251c1; font-size: 20px; font-weight: bold;"),
...
server <- function(input, output, session) {
  ...
  previousCurators <- reactiveVal(NULL) # This needs to change everytime that a new csv is openned
  
  # The curator which rows we need to fill
  finalCurator <- "Final"
  # If I put it here is easier to change it if we change the curators name
  
  # Observe the changes in the input file
  observeEvent(input$revisionFile, {
    # Read the new data from the CSV and store it in df$data
    df$data <- read.csv(input$revisionFile$name)

    # Update empty label rows and total sentences based on the new data
    empty_label_rows(which((is.na(df$data$label) | df$data$label == "") & df$data$Curator == finalCurator))
    
    total_sentences(length(empty_label_rows()))
    
    if (total_sentences() == 0){
      showNotification("No sentences without labelling for this user, choose another user", type = "error", duration = 3)
    }
    
    # Reset the row index (this should not be affected by input$person)
    row_index(1)
    
    # Get the different curators
    cleanCurators <- setdiff(unique(df$data$Curator), "Final") # We remove the one we are going to label
    previousCurators(cleanCurators)
  })
  
  # Show what file we are changing
  output$fileEdited <- renderText({HTML(paste0("You are currently editing the file: <b>", input$revisionFile$name,"</b>"))})
  ...
  # Define the text that is going to be given in the curator column
  output$previousChoices <- renderUI({
    final_text <- "" # Initialize the text
    # Get the id values of the row we are getting
    id_values_row <- df$data[empty_label_rows()[row_index()], c("pmid", "number", "indexStart", "indexEnd")]
    for (name in previousCurators()){
      final_text <- paste(final_text, "<h4><b>" , name, "</b></h4>")
      values_curator <- df$data[df$data$pmid == id_values_row$pmid & df$data$number == id_values_row$number & df$data$indexStart == id_values_row$indexStart & df$data$indexEnd == id_values_row$indexEnd & df$data$Curator == name, ]
      if (nrow(values_curator) == 0){ # This curator has not labelled this row
        final_text <- paste(final_text, "<br>Curator <b>DID NOT</b> labelled this gene<br>")
      }
      else{
        if (isTRUE(values_curator$NotGeneError)){ # Reported as non gene
          final_text <- paste(final_text, "<br>Curator reported this entity as <b>NOT GENE</b><br>")
        }
        else{# Labelled it in a way, we are going to show how
          final_text <- paste(final_text, "<br><b> -> Related to Chondrogenesis:</b> " , values_curator$label, "<br>")
          final_text <- paste(final_text, "<br><b> -> Proof:</b> " , values_curator$proof, "<br>")
          final_text <- paste(final_text, "<br><b> -> Type of info.:</b> " , values_curator$association, "<br>")
          final_text <- paste(final_text, "<br><b> -> In vivo/In vitro:</b> " , values_curator$experiment, "<br>")
        }
      }
      final_text <- paste(final_text, "<hr style='border: none; height: 1px; background-color: #999;'>")
    }
    HTML(final_text)
  })
  ...
  observeEvent(input$save, {
    ...
    # Check that if relevance is yes we need to have as well proof
      ...
      write.csv(df$data, input$revisionFile$name, row.names = FALSE)
  ...
  # We need to do it separately with the 3 check box because they are independent
  ...
  observeEvent(input$moreGenes, {
    output$dynamic_ui <- renderUI({
      tagList(
        ...
        actionButton("cancel_moreGenes", "Cancel") ### New
      )
    })
  })
