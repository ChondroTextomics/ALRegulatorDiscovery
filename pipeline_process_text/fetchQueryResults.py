...
parser.add_argument("-mindate", help = "data (year/month/day) that will bethe min date when searching in the database with entrez", required = True)
parser.add_argument("-maxdate", help = "data (year/month/day) that will bethe max date when searching in the database with entrez", required = True)
...
parser.add_argument("-query", help = "query to search in the database with entrez", required = True)
...
# Check the min and maxdate are in the correct format
...
# If we have 9999 or less we will use Entrez.search(), if it is more, we will use EDirect (command line)
if int(initial_object["Count"]) <= 9999:
    # Lets get the txt file with only the PMIDs
    with open(args.out, "w") as output_file:
        output_file.writelines(id + "\n" for id in initial_object["IdList"])
    ...
    if args.retrieve:
        print(f"Extracting information for {int(initial_object['Count'])} articles with EDirect. It could take some minutes!")
